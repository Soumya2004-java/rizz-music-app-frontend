...
class _ArtistPageState extends State<ArtistPage> {
  ...
  static const List<Map<String, String>> _fallbackArtists = [
    {
      'name': 'Arijit Singh',
      'listeners': '19.2M monthly',
  ...
  List<Map<String, String>> get _artists {
    if (widget.selectedArtists.isNotEmpty) {
      return widget.selectedArtists;
    }
    return _fallbackArtists;
  }

  List<Map<String, String>> get _visibleArtists {
    List<Map<String, String>> filtered = _artists;
    if (_selectedFilter != 0) {
      final selected = _filters[_selectedFilter];
      if (selected == 'Trending') {
        filtered = filtered
            .where((artist) => artist['tag'] == 'Trending')
            .toList();
      } else {
        filtered = filtered
            .where((artist) => artist['group'] == selected)
            .toList();
      }
    }
    ...
    return filtered.where((artist) {
      final name = (artist['name'] ?? '').toLowerCase();
      final tag = (artist['tag'] ?? '').toLowerCase();
      final group = (artist['group'] ?? '').toLowerCase();
                  ...
                  child: _glassPanel(
                    ...
                    child: TextField(
                      ...
                      decoration: InputDecoration(
                        ...
                        hintText: 'Browse your selected artists',
                        hintStyle: TextStyle(
                ...
                SliverPadding(
                  ...
                  sliver: SliverGrid(
                    ...
                    delegate: SliverChildBuilderDelegate((context, index) {
                      final artist = _visibleArtists[index];
                      return _artistCard(artist);
                    }, childCount: _visibleArtists.length),
                  ),
                ),
            ],
          ),
  ...
  Widget _artistCard(Map<String, String> artist) {
    final image = artist['image'] ?? '';
    final name = artist['name'] ?? 'Unknown';
    final listeners = artist['listeners'] ?? '';
    final tag = artist['tag'] ?? 'Artist';
